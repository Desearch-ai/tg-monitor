...
class ReadOnlyStore:
    ...
    def list_dialogs(
        ...
        sql = """
            SELECT dialog_id, MAX(dialog_name) AS dialog_name, MAX(dialog_type) AS dialog_type,
                   COUNT(*) AS message_count, MAX(date) AS latest_date
            FROM messages
        """
        if where:
            sql += " WHERE " + " AND ".join(where)
        sql += " GROUP BY dialog_id HAVING COUNT(*) >= ? ORDER BY latest_date DESC LIMIT ?"
    ...
    def get_thread(self, dialog_id: str, message_id: int, context: int = 10, max_depth: int = 20) -> dict[str, Any]:
        ...
        with self.connect() as conn:
            reply_rows = conn.execute(
                self._select_sql("dialog_id = ? AND reply_to_id = ?", "date ASC"),
                [str(dialog_id), int(message_id), clamp_limit(context, default=10)],
            ).fetchall()
            before_rows = conn.execute(
                self._select_sql("dialog_id = ? AND date < ?", "date DESC"),
                [str(dialog_id), anchor["date"], clamp_limit(context, default=10)],
            ).fetchall()
            after_rows = conn.execute(
                self._select_sql("dialog_id = ? AND date > ?", "date ASC"),
                [str(dialog_id), anchor["date"], clamp_limit(context, default=10)],
            ).fetchall()
    ...
    def _query_messages(self, where: list[str], params: list[Any], limit: int, no_text: bool = False) -> list[dict[str, Any]]:
        ...
        with self.connect() as conn:
            rows = conn.execute(self._select_sql(where_sql, "date DESC"), [*params, clamp_limit(limit)]).fetchall()
    ...
    @staticmethod
    def _select_sql(where_sql: str, order_by: str) -> str:
        return f"""
            SELECT dialog_id, dialog_name, dialog_type, msg_id, sender_name, text, date, reply_to_id
